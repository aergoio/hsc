...
local function __init__(manifestAddress)
  ...
  contract.call(_MANIFEST_ADDRESS:get(), "__init_module__",
    MODULE_NAME, scAddress)
end

local function __callFunction(module_name, func_name, ...)
  system.print(MODULE_NAME .. "__callFucntion: module_name=" .. module_name
          .. ", func_name=" .. func_name)
  return contract.call(_MANIFEST_ADDRESS:get(), "__call_module_function__",
...
function insert(sql, ...)
  system.print(MODULE_NAME .. "insert: sql=" .. sql
          .. ", args=" .. json:encode({...}))
...
function update(sql, ...)
  system.print(MODULE_NAME .. "update: sql="
          .. sql .. ", args=" .. json:encode({...}))
  local stmt = db.prepare(sql)
  stmt:exec(...)
end

function delete(sql, ...)
  system.print(MODULE_NAME .. "delete: sql="
          .. sql .. ", args=" .. json:encode({...}))
  local stmt = db.prepare(sql)
  stmt:exec(...)
end
...
function select(sql, ...)
  system.print(MODULE_NAME .. "select: sql="
          .. sql .. ", args=" .. json:encode({...}))
